...
class Position:
    ...
    def __init__(self, pk, account_pk, ticker, num_shares,equity =0):
        self.pk = pk
    ...
    def _insert(self):
        with sqlite3.connect(self.dbname) as conn:
            ...
            SQL = """INSERT INTO positions 
                    ...
                    VALUES (?,?,?,?);"""
            values = (self.account_pk, self.ticker, self.num_shares,self.equity)
    ...
    def _update(self):
         with sqlite3.connect(self.dbname) as conn:
            ...
            values = (self.num_shares,self.equity, self.account_pk, self.ticker)
            cursor.execute(SQL, values)

    
    def equity_update(self):
        with sqlite3.connect(self.dbname) as conn:
            cursor = conn.cursor()
    ...
    def all_for_account(cls, pk):
        with sqlite3.connect(cls.dbname) as conn:
            cursor = conn.cursor()
            SQL = """SELECT ticker, number_shares,equity FROM positions WHERE account_pk=? AND number_shares >0"""
    ...
    def from_account(cls, pk, ticker):
        with sqlite3.connect(cls.dbname) as conn:
            cursor = conn.cursor()
            SQL = """SELECT * FROM positions WHERE account_pk=? AND ticker=?"""
            values = (pk, ticker)
            cursor.execute(SQL, values)
